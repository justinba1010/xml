/* OASIS_START */
/* DO NOT EDIT (digest: 32e352519c7720f4a113f19ece206a2b) */
module OASISGettext = {
  ...
  */
  ...
  let exec_from_conf = exec => {
    let exec = {
      ...
      try (BaseEnvLight.var_get(exec, env)) {
      | Not_found =>
    ...
    let fix_win32 = str =>
      if (Sys.os_type == "Win32") {
        let buff = Buffer.create(String.length(str));
  ...
  let find_packages = () =>
    List.map(
      before_space,
      split_nl & run_and_read(exec_from_conf("ocamlfind") ++ " list"),
    );
  ...
  let dispatch = conf =>
    ...
    | After_rules => {
        ...
        List.iter(
          pkg => {
            let base_args = [A("-package"), A(pkg)];
            /* TODO: consider how to really choose camlp4o or camlp4r. */
            let syn_args = [A("-syntax"), A("camlp4o")];
            let (args, pargs) =
              /* Heuristic to identify syntax extensions: whether they end in
                    ".syntax"; some might not.
                 */
              if (!conf.no_automatic_syntax
                  && (
                    Filename.check_suffix(pkg, "syntax")
                    || List.mem(pkg, well_known_syntax)
                  )) {
            ...
            flag(["ocaml", "infer_interface", "package(" ++ pkg ++ ")"])
            & S(pargs);
          },
          find_packages(),
        );
        ...
        flag(
          ["ocaml", "pkg_threads", "doc"],
          S([A("-I"), A("+threads")]),
        );
        flag(["ocaml", "pkg_threads", "link"], S([A("-thread")]));
        flag(
          ["ocaml", "pkg_threads", "infer_interface"],
          S([A("-thread")]),
        );
        flag(["c", "pkg_threads", "compile"], S([A("-thread")]));
        flag(["ocaml", "package(threads)", "compile"], S([A("-thread")]));
        flag(
          ["ocaml", "package(threads)", "doc"],
          S([A("-I"), A("+threads")]),
        );
        flag(["ocaml", "package(threads)", "link"], S([A("-thread")]));
        flag(
          ["ocaml", "package(threads)", "infer_interface"],
          S([A("-thread")]),
        );
...
module MyOCamlbuildBase = {
  ...
  type t = {
    lib_ocaml: list((name, list(dir), list(string))),
    lib_c: list((name, dir, list(file))),
    flags: list((list(tag), OASISExpr.choices(spec))),
    /* Replace the 'dir: include' from _tags by a precise interdepends in
     * directory.
     */
    includes: list((dir, list(dir))),
  };
  ...
  let dispatch_combine = (lst, e) =>
    List.iter(dispatch => dispatch(e), lst);

  let tag_libstubs = nm => "use_lib" ++ nm ++ "_stubs";

  let nm_libstubs = nm => nm ++ "_stubs";
  ...
  let dispatch = (t, e) => {
    ...
    | Before_options =>
      ...
      List.iter(
        ((opt, var)) =>
          try (opt := no_trailing_dot(BaseEnvLight.var_get(var, env))) {
          | Not_found => Printf.eprintf("W: Cannot get variable %s\n", var)
          },
        [
    ...
    | After_rules =>
      ...
      List.iter(
        ...
        | (nm, [], intf_modules) => {
            ...
            let cmis =
              List.map(
                m => OASISString.uncapitalize_ascii(m) ++ ".cmi",
                intf_modules,
              );
            dep(["ocaml", "link", "library", "file:" ++ nm ++ ".cma"], cmis);
          }
        | (nm, [dir, ...tl], intf_modules) => {
            ocaml_lib(~dir, dir ++ "/" ++ nm);
            List.iter(
              dir =>
                List.iter(
                  str =>
      ...
      List.iter(
        ((dir, include_dirs)) => Pathname.define_context(dir, include_dirs),
        t.includes,
      );
      ...
      List.iter(
        ((lib, dir, headers)) => {
          ...
          if (bool_of_string(BaseEnvLight.var_get("native_dynlink", env))) {
            flag(
          ...
          flag(["link", "ocaml", "use_" ++ lib], S([A("-I"), P(dir)]));
        },
        t.lib_c,
      );
      ...
      List.iter(
        ((tags, cond_specs)) => {
          let spec = BaseEnvLight.var_choose(cond_specs, env);
          let rec eval_specs =
            fun
  ...
  let dispatch_default = (conf, t) =>
    dispatch_combine([dispatch(t), MyOCamlbuildFindlib.dispatch(conf)]);
};

open Ocamlbuild_plugin;
let package_default = {
  MyOCamlbuildBase.lib_ocaml: [("erm_xml", ["src"], [])],
  lib_c: [],
  flags: [],
  includes: [("tests", ["src"])],
};
